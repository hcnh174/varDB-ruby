...
Ext.ux.vardb.explorer.RegexDialog = Ext.extend(Ext.Window,
{	
	...
	initComponent:function()
	{
		...
		this.sequencegrid=new Ext.ux.vardb.explorer.regex.SequenceTable({controller: controller, list_id: list_id, pagesize: this.pagesize, onSuccess: onSequencesAdded});
...
Ext.ux.vardb.explorer.regex.FormTab=function(controller,dialog,config)
{
	var form;
...
Ext.ux.vardb.explorer.regex.RegexTab=function(controller,dialog)
{
	var config=
	{
		type: 'REGEX',
...
Ext.ux.vardb.explorer.regex.PrositeTab=function(controller,dialog)
{
	var config=
	{
		type: 'PROSITE',
...
Ext.ux.vardb.explorer.regex.AminoAcidTab=function(controller,dialog)
{
	var config=
	{
		type: 'AA',
...
Ext.ux.vardb.explorer.regex.SequenceTable = Ext.extend(Ext.ux.vardb.Grid,
{	
	initComponent:function()
	{
		var self=this;
		...
		{
			return Ext.ux.vardb.Renderer.renderTags(value,p,r,this.list_id);
		}
		...
		var addAsNewcartButton=
		{
			...
			formBind: true,
			scope: this,
			handler: function(){Ext.ux.vardb.explorer.Services.createSubset(self,self.controller,self.onSuccess);}
		};
				...
				[
					{
						...
						handler: function()
						{
							//var dialog=new Ext.ux.vardb.tags.ApplyTagDialog(self,self.onTagSequences);
...
Ext.ux.vardb.explorer.regex.Expander=function(maxcolumns)
{
...
Ext.ux.vardb.explorer.regex.MatchTable = Ext.extend(Ext.ux.vardb.Grid,
{	
	initComponent:function()
	{
		var reader=new Ext.data.JsonReader(
		...
		{
			return String.format('<a href="#" onclick="new Ext.ux.vardb.explorer.WebLogo({identifier: \'{1}\', regex: \'{2}\'})">{0}</span>',
